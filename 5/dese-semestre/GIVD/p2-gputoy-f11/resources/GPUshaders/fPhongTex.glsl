...
in vec2 TexCoords; // Afegim textures
...
uniform sampler2D textureMap;                  // Mapa de textures
...
void main()
{
    ...
    // Aplicar textura (si és necessari)
    ...
    for (int i = 0; i < MAX_GPU_LIGHTS; ++i)
    {
        if (lightComponents[i].enabled) // Processar només llums actives
        {
            ...
            vec3 diffuseColor = mix(materialComponents.Kd, textureColor, 0.5); // Aplicar textura a la component difusa
            vec3 diffuse = NL * lightComponents[i].Id * diffuseColor;
            ...
            // Aquí es pot aplicar la textura a la component especular
