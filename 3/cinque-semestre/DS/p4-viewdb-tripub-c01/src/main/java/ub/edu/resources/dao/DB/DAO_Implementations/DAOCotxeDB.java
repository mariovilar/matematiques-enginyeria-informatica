...
import ub.edu.model.Cotxe;
import ub.edu.resources.dao.DAO_Interfaces.DAOCotxe;
...
public class DAOCotxeDB implements DAOCotxe {
    ...
    public DAOCotxeDB(Connection c) {
        ...
        if(this.array_AvailableTablesNames.contains("Cotxe")){
            this.nameTable = "Cotxe";
            ...
            System.out.println("\n*****************************************************************");
            System.out.println("ERROR en DAOCotxeDB al intentar abrir el nombre de la tabla");
            System.out.println("*****************************************************************");
        }
    }

    private List<Cotxe> convertir_array_de_Hasmaps_a_List(ArrayList<Object> arr){
        List<Cotxe> list = new ArrayList<>();
        for(Object obj: arr){
            HashMap<Object,Object> hashMap = (HashMap<Object,Object>) obj;
            Cotxe c = new Cotxe();

            if(hashMap.containsKey("id_cotxe")){c.setId((Integer) hashMap.get("id_cotxe"));}
            else{c.setId(null);}

            if(hashMap.containsKey("model")){c.setModel((String) hashMap.get("model"));}
            else{c.setModel(null);}

            if(hashMap.containsKey("tipusCombustible")){c.setTipusCombustible((Boolean) hashMap.get("tipusCombustible"));}
            else{c.setTipusCombustible(null);}
    ...
    private void rellenarMadreEnLaHija(Transport madre, Cotxe hija){
    ...
    @Override
    public Optional<Cotxe> getById(String id) throws Exception {
        ...
        ArrayList<Object> arr = this.crud_tables.select_OneParam(this.nameTable,"id_cotxe",Integer.valueOf(id));
        //System.out.println("arr:"+arr);
        List<Cotxe> list = this.convertir_array_de_Hasmaps_a_List(arr);
        //System.out.println("list:"+list);
        if(list.size()>1){
            System.out.println("ERROR: Existe mas de 1 cotxe con el mismo id");
        }
        if(!list.isEmpty()){
    ...
    @Override
    public List<Cotxe> getAll() throws Exception {
        ArrayList<Object> arr = this.crud_tables.select_all(this.nameTable);
        List<Cotxe> list = this.convertir_array_de_Hasmaps_a_List(arr);
        List<Cotxe> listFinal = new ArrayList<>();
        for (Cotxe cotxe: list) {
            ...
            Cotxe cotxeFinal = this.getById(cotxe.getId().toString()).get();
            listFinal.add(cotxeFinal);
        }
    ...
    @Override
    public boolean add(Cotxe cotxe) throws Exception {
        HashMap<Object, Object> hashmap = new HashMap<Object, Object>();
        hashmap.put("model",cotxe.getModel());
        hashmap.put("tipusCombustible",cotxe.isTipusCombustible());

        //como es una herencia, primero hay que añadir la clase madre
        boolean resuMadre = this.daoTransportDB.add((Transport) cotxe);
        Transport transport = this.daoTransportDB.getTransport((Transport) cotxe).get();
    ...
    @Override
    public boolean update(Cotxe cotxe, String[] params) throws Exception {
        HashMap<Object, Object> hashmap_OLD = new HashMap<Object, Object>();
        hashmap_OLD.put("id_cotxe",cotxe.getId());
        ...
        if(!flag_pk){
            hashmap_NEW_hija.put("id_cotxe",cotxe.getId());
        }

        ArrayList<Object> array_AutoInc_PKs = new ArrayList<>();
        array_AutoInc_PKs.add("id_cotxe");
        ...
        boolean resuMadre = this.daoTransportDB.update((Transport) cotxe, newArray);
        System.out.println(resuMadre);
        return (resu && resuMadre);
    }

    @Override
    public boolean delete(Cotxe cotxe) throws Exception {
        HashMap<Object, Object> hashmap = new HashMap<Object, Object>();
        hashmap.put("id_cotxe",cotxe.getId());
        boolean resu = this.crud_tables.delete(this.nameTable,hashmap);
        System.out.println(resu);
        //una vez borrada la hija, tambien borramos la madre
        boolean resuMadre = this.daoTransportDB.delete((Transport) cotxe);
