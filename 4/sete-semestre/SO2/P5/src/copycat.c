...
void read_airports_data(int **num_flights, char **airports, char *fname, int num_lines, int num_threads) 
{
  ...
  while(1) {
    ...
    while ((cellCambio->nelems < MAX_LINES) && (fgets(cellCambio->lines[cellCambio->nelems], MAXCHAR, fp) != NULL)) {
      cellCambio->nelems++;
    }

    // Añadimos el buffer siempre y cuando la cell no esté vacía
    if(cellCambio->nelems > 0) {
      ...
      // Si la celda está vacía salimos del bucle
      break;
    }
  }
