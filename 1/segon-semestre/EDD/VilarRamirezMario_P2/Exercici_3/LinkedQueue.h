...
public:
    LinkedQueue();
    LinkedQueue(const LinkedQueue<E>& q);
    ~LinkedQueue();
...
LinkedQueue<E>::LinkedQueue(const LinkedQueue<E>& q)
{
    ...
    else
    {
        ...
        {
            nNode->setNext(new DoubleNode<E>(qNode->getNext()->getElement())); //setNext
            nTmp = nNode;
            qNode = qNode->getNext();
            nNode = nNode->getNext();
            nNode -> setPrevious(nTmp); //setPrevious
            _size++;
        }
        this->_back = nNode;
    }
...
// En aquest exercici, ens centrarem en usar el print() de Patient
template <class E>
void LinkedQueue<E>::print()const
{
    ...
    {
        cout << "Pacient " << i << ": ";
        cout << (node->getElement())->print() << endl;
        node = node->getNext();
        i++;
    }
}
